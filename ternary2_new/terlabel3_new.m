...
if nargout >= 1
    handles = ones(3, 1);
    handles(1) = text(-0.05, -0.06, label2, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Bottom left vertex
    handles(2) = text(1.05, -0.06, label3, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Bottom right vertex
    handles(3) = text(0.5, sqrt(3)/2 + 0.06, label1, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Top vertex
else
    text(-0.05, -0.06, label2, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Bottom vertex
    text(1.05, -0.06, label3, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Right vertex
    text(0.5, sqrt(3)/2 +0.06 , label1, 'HorizontalAlignment', 'center', 'FontSize', fontSize); % Top vertex
end

%{ 
when labels lie on middle of each edge
if nargout >= 1
    handles = ones(3, 1);
